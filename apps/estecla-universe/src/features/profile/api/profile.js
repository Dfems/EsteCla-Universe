...
function validateAndNormalizeUsername(username) {
...
async function ensureUsernameIfChanged(current, usernameLowercase) {
...
async function maybeUploadProfilePic(uid, file) {
    if (!file)
        return '';
...
function computeBirthdayUpdate(birthday) {
    ...
    if (age < 13)
        throw new Error('Devi avere almeno 13 anni.');
...
function valueOrDelete(value) {
    const v = (value || '').toString().trim();
    return v ? v : deleteField();
}
