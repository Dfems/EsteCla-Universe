    ...
    useEffect(() => {
        ...
        const init = async () => {
            try {
                ...
                const userQueryLower = query(collection(db, 'users'), where('usernameLowercase', '==', username.toLowerCase()));
                ...
                unsubscribeUser = onSnapshot(userRef, (snap) => {
                    if (snap.exists()) {
                        ...
                        if (unsubscribePosts)
                            unsubscribePosts();
                        unsubscribePosts = onSnapshot(query(collection(db, 'users', data.uid, 'posts'), orderBy('createdAt', 'desc')), (postsSnap) => {
                            const toDate = (v) => {
                                if (!v)
                                    return undefined;
