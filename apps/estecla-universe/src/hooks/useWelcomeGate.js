...
const storageKey = (uid) => (uid ? `welcome.lastSeenAt:${uid}` : 'welcome.lastSeenAt');
const snoozeKey = (uid) => (uid ? `welcome.snoozeUntil:${uid}` : 'welcome.snoozeUntil');
export function useWelcomeGate() {
    ...
    const getSnoozeUntil = useCallback(() => {
        try {
